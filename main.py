...
# -*- coding: utf-8 -*-
"""
Crypto Exam Generator (v1.8e-b)

Hotfix spustitelnosti bez změny požadovaného chování 1.8e:
- oprava "nebo" -> "or" (SyntaxError) v `_add_subgroup`
- odstraněna rekurze (`_save_current_q` už nevolá `save_data`)
- "Uložit změny otázky" nyní uloží i na disk (volá `save_data` jen z handleru tlačítka)
- obnoven `DnDTree` (drag & drop) v levém stromu
"""
...
class DnDTree(QTreeWidget):
    ...
    def dropEvent(self, event) -> None:
        ...
        # pokud by se někdy mělo perzistovat pořadí, zde by šla sync routine
...
class MainWindow(QMainWindow):
    ...
    def _connect(self):
        ...
        self.btn_save_q.clicked.connect(lambda: (self._save_current_q(False), self.save_data()))
